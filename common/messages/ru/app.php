<?php

return [
    'Username' => 'Имя пользователя',
    'Login' => 'Войти',
    'Log in' => 'Вход',
    'or' => 'Или',
    'Home' => 'Главная',
    'If you forgot your password you can' => 'Если вы забыли пароль, то вы можете ',
    'Password' => 'Пароль',
    'reset it' => 'восстановить его',
    'Signup' => 'Зарегистрироваться',
    'Need new verification email?' => 'Нужен новый верификационный email?',
    'Remember Me' => 'Запомнить меня',
    'Resend' => 'Послать еще раз',
    'Send' => 'Отправить',
    'Join us' => 'Зарегистрироваться',
    'Notifications' => 'Уведомления',
    'What do you want to write,' => 'Что хотите написать,',
    'Log out' => 'Выйти',
    'Phone' => 'Телефон',
    'Upload Image' => 'Загрузить Картинку',
    'Save' => 'Сохранить',
    'Publish' => 'Опубликовать',
    'Settings' => 'Настройки',
    'You' => 'Вы',
    'Subscribers' => 'Подписчики',
    'Subscribed' => 'Подписки',
    'Your avatar was successfully changed' => 'Ваша картинка успешно изменена',
    'Date' => 'Дата',
    'Sorry we did not find what you wanted' => 'Извините, но мы не нашли что вы хотели',
    'Period' => 'Период',
    'Commented' => 'Комментируемые',
    'Your comment was deleted' => 'Ваш комментарий был удален',
    'Change language' => 'Изменить язык',
    'Change password' => 'Изменить пароль',
    'Change avatar' => 'Изменить аватарку',
    'Edit your content here...' => 'Редактируйте свой контент здесь...',
    'All time' => 'За все время',
    'Day' => 'За день',
    'Week' => 'За неделю',
    'Month' => 'За месяц',
    'Author' => 'Автор',
    'My profile' => 'Мой профиль',
    'Liked' => 'Понравившееся',
    'Starred' => 'Закладки',
    'Log in with' => 'Войти с помощью',
    'Signup with' => 'Зарегистрироваться с помощью',
    'Tech Support' => 'Техподдержка',
    'Name' => 'Название',
    'Subject' => 'Причина',
    'Body' => 'Описание',
    'Type' => 'Тип',
    'Post' => 'Пост',
    'Update' => 'Редактировать',
    'Profile' => 'Профиль',
    'Delete' => 'Удалить',
    'Comment' => 'Комментарий',
    'Commentate' => 'Комментировать',
    'Comments' => 'Комментарии',
    'Repost' => 'Репост',
    'Like' => 'Лайк',
    'No comments' => 'Нет комментариев',
    'The comment is successfully added!' => 'Комментарий успешно добавлен!',
    'Reply' => 'Ответить',
    'Number of records' => 'Количество записей',
    'Description' => 'Описание',
    'Image' => 'Картинка',
    'If you have any questions please contact' => 'Если у вас есть какие-то вопросы пожалуйста обращайтесь',
    'Registration' => 'Регистрация',
    'Your email has been confirmed!' => 'Ваш email был подтвержден!',
    'Hello' => 'Здравствуйте',
    'Sorry, we are unable to verify your account with provided token.' => 'Извините мы не можем проверить вашу почту по предоставленному токену.',
    'First Name' => 'Имя',
    'Posts' => 'Посты',
    'Edit Profile' => 'Редактировать данные профиля',
    'Thank you for registration. Please check your inbox for verification email.' => 'Спасибо за регистрацию. Пожалуйста 
    проверте ваши входящие сообщения для подтверждения почты.',
    'Last Name' => 'Фамилия',
    'Update Profile' => 'Редактировать профиль',
    'This email address has already been taken.' => 'Этот email уже был взят',
    'Follow the link below to verify your email' => 'Проследуйте по этой ссылке, чтобы подтвердить ваш email',
    'We did not find the page you are searching for' => 'Мы не нашли страницу которую вы искали',
    'Perhaps you are searching' => 'Возможно вы искали',
    'main page' => 'главную страницу',
    'Please fill out your email. A link to reset password will be sent there.' => 'Пожалуйста, заполните вашу электронную почту. Ссылка для сброса пароля будет отправлена туда.',
    'Request password reset' => 'Запросить сброс пароля',
    'Resend verification email' => 'Прислать еще раз верификационный email',
    'Please fill out your email. A verification email will be sent there.' => 'Пожалуйста, заполните вашу электронную почту. На это будет отправлено письмо с подтверждением.',
    'What are you searching?' => 'Что сегодня ищем?',
    'Please fill the fields below to signup' => 'Пожалуйста заполните следующие поля для регистрации',
    'Please fill the fields below to login' => 'Пожалуйста заполните следующие поля для входа',
];
